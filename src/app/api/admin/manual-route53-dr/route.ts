...
import {
  ...
  resolveManualRoute53DrFromAdmin,
} from "@/lib/manual-route53-dr-resolve";
...
async function stsOr503() {
  try {
    ...
    console.error("[manual-route53-dr] STS probe failed:", err.code ?? "(no code)", err.message, err.stderr ?? "");
...
export async function GET(req: Request) {
  ...
  const home = ssmHome();
  const enabled = await resolveManualRoute53DrFromAdmin(home);
...
export async function POST(req: Request) {
  ...
  const resolved = await resolveManualRoute53DrFromAdmin(home);
  return NextResponse.json({ ok: true, enabled: resolved });
}
